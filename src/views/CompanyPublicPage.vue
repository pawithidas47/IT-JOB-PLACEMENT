...
export default {
  ...
  async mounted() {
    ...
    try {
      ...
      console.error("❌ โหลดข้อมูลสาธารณะล้มเหลว:", e);
    } finally {
      this.loaded = true;
    }
  },
...
.grid{
  ...
  gap:16px;
  align-items:start;  /* ไม่ยืดสูงเท่ากัน */
}
...
.job-item{
  ...
  border-radius:12px;
  box-shadow:0 8px 20px rgba(16,24,40,.05);
  padding:14px 16px 46px;         /* เผื่อพื้นที่วันที่ล่างขวา */
  cursor:pointer;
...
.tag{
  ...
  padding:3px 10px;font-size:.86rem;line-height:1;border-radius:999px;
  background:#fff5e6;border:1px solid #ffb380;color:#ff6600;font-weight:700;white-space:nowrap
}

/* เมตา: เงิน/จำนวน คนละบรรทัด */
.job-meta-col{
  display:grid;grid-template-columns:1fr;row-gap:6px;color:#4b5563
}
