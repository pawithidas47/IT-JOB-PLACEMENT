        ...
        <div class="toolbar">
          ...
          <div class="tool-right">
            ...
            <select v-model="sortOption" class="select-pill">
              <option value="date">ทั้งหมด</option>
...
export default {
  ...
  data() {
    return {
      ...
      defaultProfile,
      searchQuery: "",
      // ค่าเริ่มต้นเรียงตาม "ใหม่ล่าสุด"
      sortOption: "date",
    };
  },
  computed: {
    filteredApplicants() {
      ...
      // 2) กรองสถานะตามตัวเลือก
      if (["approved", "rejected", "pending"].includes(this.sortOption)) {
        list = list.filter(a => (a.status || "pending") === this.sortOption);
        // เรียงใหม่ล่าสุดด้วยเสมอเมื่อกรองสถานะ
        list.sort((a, b) => new Date(b.applied_at) - new Date(a.applied_at));
        return list;
      }

      // 3) เรียงตามตัวเลือกทั่วไป
...
/* ===== Page head ===== */
.page-head{display:flex;flex-direction:column;gap:.6rem;margin-bottom:1rem}
...
.tool-right{display:flex;align-items:center;gap:.6rem;flex-wrap:wrap}
...
.status-pill{padding:.38rem .75rem;border-radius:999px;font-size:.82rem;font-weight:600;border:1px solid transparent}
